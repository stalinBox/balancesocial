...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>estrategia/nuevoEstrategiaEfecto"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Efecto</th>					
				...
				<tbody>
					<?php 
					$filaAlterada = 0;
					if (!empty($estrategiaEfectos)) {
						foreach ($estrategiaEfectos as $key): 
							$id   = base64_encode($key->IdEfecto); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->Efecto ?></th>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>estrategia/editarEstrategiaEfecto/<?php echo $key->IdEfecto;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarEstrategiaEfecto('<?php echo $key->IdEfecto; ?>','<?php echo $key->Efecto; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Efecto</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					   <body onload="viewdata('Estrategia/filtroAnioEstrategiaEfecto','Estrategia/filtroAnioGuardarEstrategiaEfecto')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarEstrategiaEfecto(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idEstrEfe=id;
				$.post('<?php echo base_url(); ?>estrategia/eliminarEstrategiaEfecto',{idEstrEfe: idEstrEfe},function(data) {
				location.reload();
				});
			}
		});
