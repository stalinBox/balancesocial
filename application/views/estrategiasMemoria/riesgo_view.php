...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>estrategia/nuevoEstrategiaRiesgo"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Riesgo</th>					
				...
				<tbody>
					<?php 
					$filaAlterada = 0;
					if (!empty($estrategiaRiesgos)) {
						foreach ($estrategiaRiesgos as $key): 
							$id   = base64_encode($key->IdRiesgo); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->Riesgo ?></th>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>estrategia/editarEstrategiaRiesgo/<?php echo $key->IdRiesgo;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarEstrategiaRiesgo('<?php echo $key->IdRiesgo; ?>','<?php echo $key->Riesgo; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Riesgo</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('Estrategia/filtroAnioEstrategiaRiesgo','Estrategia/filtroAnioGuardarEstrategiaRiesgo')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarEstrategiaRiesgo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idRie=id;
				$.post('<?php echo base_url(); ?>estrategia/eliminarEstrategiaRiesgo',{idRie: idRie},function(data) {
				location.reload();
				});
			}
		});
