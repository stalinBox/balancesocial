...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>estrategia/nuevaEstrategiaPolitica"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Política</th>					
				...
				<tbody>
					<?php 
					$filaAlterada = 0;
					if (!empty($estrategiaPoliticas)) {
						foreach ($estrategiaPoliticas as $key): 
							$id   = base64_encode($key->IdPolitica); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->Politica ?></th>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>estrategia/editarEstrategiaPolitica/<?php echo $key->IdPolitica;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarEstrategiaPolitica('<?php echo $key->IdPolitica; ?>','<?php echo $key->Politica; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Política</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('Estrategia/filtroAnioEstrategiaPolitica','Estrategia/filtroAnioGuardarEstrategiaPolitica')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarEstrategiaPolitica(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idPol=id;
				$.post('<?php echo base_url(); ?>estrategia/eliminarEstrategiaPolitica',{idPol: idPol},function(data) {
				location.reload();
				});
			}
		});
