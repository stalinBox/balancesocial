...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>estrategia/nuevaEstrategiaOportunidad"
	}
	function regresar(){
		window.location="<?php echo base_url()?>estrategia";
	}
	function denegado() {
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Oportunidad</th>					
				...
				<tbody>
					<?php 
					if (!empty($estrategiaOportunidades)) {
						foreach ($estrategiaOportunidades as $key): 
							$id   = base64_encode($key->IdOportunidad); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->Oportunidad ?></th>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>estrategia/editarEstrategiaOportunidad/<?php echo $key->IdOportunidad;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarEstrategiaOportunidad('<?php echo $key->IdOportunidad; ?>','<?php echo $key->Oportunidad; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Oportunidad</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					   <body onload="viewdata('Estrategia/filtroAnioEstrategiaOportunidad','Estrategia/filtroAnioGuardarEstrategiaOportunidad')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarEstrategiaOportunidad(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idOpor=id;
				$.post('<?php echo base_url(); ?>estrategia/eliminarEstrategiaOportunidad',{idOpor: idOpor},function(data) {
				location.reload();
				});
			}
		});
