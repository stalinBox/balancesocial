...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>memoria/nuevoDetalleMemoria"
	}
	function regresar(){
		window.location="<?php echo base_url()?>memoria";
	}
	function denegado() {
...
<?php 
/*
echo "Permisos Detalles de Memoria";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Periodo Objeto de la Memoria</th>					
						<th scope="col" class="clmn1">Fecha de Última Memoria</th>
						<th scope="col" class="clmn1">Ciclo de Presentación de Memorias</th>
				...
				<tbody>
					<?php 
					if (!empty($detallesMemoria)) {
						foreach ($detallesMemoria as $key): 
							$id   = base64_encode($key->IdDetalleMemoria); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->PeriodoObjeto ?></th>
							<th scope="col" class="clmn1"><?php echo $key->FechaUltimaMemoria ?></th>
							<th scope="col" class="clmn1"><?php echo $key->CicloPresentacion ?></th>
							<th scope="col" class="clmn1"><?php echo $key->FechaSistema ?></th>
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>memoria/editarDetalleMemoria/<?php echo $key->IdDetalleMemoria;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarDetalleMemoria('<?php echo $key->IdDetalleMemoria; ?>','<?php echo $key->PeriodoObjeto; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
					 ...
					 <div class="input-group col-md-12">
						...
						<select id="selectPeriodo" name="selectPeriodo" class="form-control" onchange="FilterByAnios(this.value)">
							...
							<?php 
								...
								<option> <?php echo $key->PeriodoObjeto; ?></option>									
							<?php } ?>
						</select>
					 </div><br>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Ciclo de presentación de la memoria</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('Memoria/filtroAnioMemoriaDetalle','Memoria/filtroAnioGuardarMemoriaDetalle')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarDetalleMemoria(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdDetalleMem=id;
				$.post('<?php echo base_url(); ?>memoria/eliminarDetalleMemoria',{IdDetalleMem: IdDetalleMem},function(data) {
				location.reload();
				});
			}
		});
