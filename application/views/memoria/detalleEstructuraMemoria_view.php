...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>memoria/nuevaEstructuraDeMemoria"
	}
	function regresar(){
...
?>
	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				...
				<tbody>
					<?php 
					if (!empty($detallesEstructuraDeMemoria)) {
						foreach ($detallesEstructuraDeMemoria as $key): 
							$id   = base64_encode($key->IdEstructuraMemoria); ?>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>memoria/editarEstructuraMemoria/<?php echo $key->IdEstructuraMemoria;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarEstructuraMemoria('<?php echo $key->IdEstructuraMemoria; ?>','<?php echo $key->FechaSistema; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
...
<script type="text/javascript">
	...
	function eliminarEstructuraMemoria(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idDetEstMem=id;
				$.post('<?php echo base_url(); ?>memoria/eliminarEstructuraMemoria',{idDetEstMem: idDetEstMem},function(data) {
				location.reload();
				});
			}
		});
