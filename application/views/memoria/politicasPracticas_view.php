...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>memoria/nuevaPoliticaPractica"
	}
	function regresar(){
...
<?php 
/*
echo "Permisos Políticas y Prácticas";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Política o Práctica Vigente</th>					
				...
				<tbody>
					<?php 
					if (!empty($memoriaPoliticaPracticas)) {
						foreach ($memoriaPoliticaPracticas as $key): 
							$id   = base64_encode($key->IdPoliticaPractica); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->PoliticaPractica ?></th>
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>memoria/editarPoliticaPractica/<?php echo $key->IdPoliticaPractica;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarPoliticaPractica('<?php echo $key->IdPoliticaPractica; ?>','<?php echo $key->PoliticaPractica; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Política/Práctica</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('memoria/filtroAnioPoliticasPracticas','memoria/filtroAnioGuardarPoliticasPracticas')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarPoliticaPractica(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var iddPolPra=id;
				$.post('<?php echo base_url(); ?>memoria/eliminarPoliticaPractica',{iddPolPra: iddPolPra},function(data) {
				location.reload();
				});
			}
		});
