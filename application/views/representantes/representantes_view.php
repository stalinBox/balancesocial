...
<?php 
/*
echo "Permisos Representantes y O.E. ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Inicio de Actividades</th>	
					<th scope="col" class="clmn1">Fin de Actividades</th>	
					<th scope="col" class="clmn1">Cargo</th>
			...
			<tbody>
				...
				if (!empty($representantes)) {
					foreach ($representantes as $key):
						$id   = base64_encode($key->IdRepresentante);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->CedulaRepresentante ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ApellidoPaternoRepresentante." ".$key->ApellidoMaternoRepresentante. " ".$key->NombresRepresentante ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Organismo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaInicioPeriodo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaFinPeriodo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Cargo ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>representante/editarRepresentante/<?php echo $key->IdRepresentante;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarRepresentante('<?php echo $key->IdRepresentante; ?>','<?php echo $key->CedulaRepresentante ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarRepresentante(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdRepresent = id;
				$.post('<?php echo base_url(); ?>representante/eliminarRepresentante',{IdRepresent: IdRepresent},function(data) {
				location.reload();
				});
			}
		});
