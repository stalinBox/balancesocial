...
<div id="MANIPULACIONDATOS">
	<div id="menumanipulaciondatos">
		...
		<ul class="manipdatos2">
			<li> 
				<a href="#" onclick="ocultarColumna(4,true)" title="Editar"><img src="<?php echo base_url()?>imagenes/EDITAR.png" width="20" height="20" alt=""/></a>
			</li>
		</ul>
		<ul class="manipdatos">
			<li> 
				<a href="#" onclick="ocultarColumna(5,true)" title="Eliminar"><img src="<?php echo base_url()?>imagenes/BORRAR-MENU.png" width="20" height="20" alt=""/></a>
			</li>
		</ul>
		<ul class="manipdatos2">
			<li> 
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Representante</th>
					<th scope="col" class="clmn1">Organismo</th>
					<th scope="col" class="clmn1">Fecha inicio del Periodo</th>										
					<th scope="col" class="clmn1">Fecha fin del Periodo</th>	
			...
			<tbody>
				<?php 
				if (!empty($representanteOrganismos)) {
					foreach ($representanteOrganismos as $key):
						$id   = base64_encode($key->IdRepOrg);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->IdRepresentante ?></th>
						<th scope="col" class="clmn1"><?php echo $key->IdOrganismo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaPeriodoInicio ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaPeriodoFinaliza ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>representante/editarRepOrganismo/<?php echo $key->IdRepOrg;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarRepOrganismo('<?php echo $key->IdRepOrg; ?>','<?php echo $key->IdRepresentante ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarRepOrganismo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdRepresent = id;
				$.post('<?php echo base_url(); ?>representante/eliminarRepOrganismo',{IdRepresent: IdRepresent},function(data) {
				location.reload();
				});
			}
		});
