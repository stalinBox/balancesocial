	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				<?php 
				if (!empty($sindicatos)) {
					foreach ($sindicatos as $key):
						$id   = base64_encode($key->IdSindicato);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombreSindicato ?></th>
						<th scope="col" class="clmn1"><?php echo $key->DescripcionSindicato ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>sindicato/editarSindicato/<?php echo $key->IdSindicato;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSindicato('<?php echo $key->IdSindicato; ?>','<?php echo $key->NombreSindicato ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarSindicato(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSind = id;
				$.post('<?php echo base_url(); ?>sindicato/eliminarSindicato',{IdSind: IdSind},function(data) {
				location.reload();
				});
			}
		});
