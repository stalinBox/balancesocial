...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>datoBase/nuevoDatoBase";
	}
	function regresar(){
...
<div id="MANIPULACIONDATOS">
	<div id="menumanipulaciondatos">
		<ul class="manipdatos">
			<li> 
				<a href="#" <?php if (@$existeRegistro > 0) {	echo 'onclick="registrado()"';}else{ if (@$permisos[0]->Agregar == 1 OR @$admin == 1) {	echo 'onclick="nuevo()"';	}else{ echo 'onclick="denegado()"';	} } ?> title="Agregar"><img src="<?php echo base_url()?>imagenes/NUEVO.png" width="20" height="20" alt=""/></a>
			</li>
		</ul>
	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				<?php 
				if (!empty($datosBase)) {
					foreach ($datosBase as $key):
						$id   = base64_encode($key->IdDatosBase);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->IdDatosBase ?></th>
						...
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>datoBase/editarDatoBase/<?php echo $key->IdDatosBase;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarDatoBase('<?php echo $key->IdDatosBase ?>','<?php echo $key->IdDatosBase ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true" id="modalNotif">
      <div class="modal-dialog">
        <div class="modal-content">
           <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              <h3>El Registro en Datos Base se lo realiza una sola vez</h3>
           </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-info" id="modal-btn-ok">Ok</button>
      </div>
    </div>
  </div>
</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarDatoBase(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idDatoBase = id;
				$.post('<?php echo base_url(); ?>datoBase/eliminarDatoBase',{idDatoBase: idDatoBase},function(data) {
				location.reload();
				});
			}
		});
