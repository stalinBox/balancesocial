		...
		<table id="tablaPaginada" border="0" cellspacing="5px" width="100%">
			<thead>
				<tr class="fila1">
					...
					<td class="columna1">Resultado</td>
					<td class="columna1">Dimensión</td>					
					<td class="columna1">Sub-Dimensión</td>					
					<td class="columna1">Grupo de Interés</td>					
					<td class="columna1">Herramienta</td>					
					<td class="columna1">Área</td>					
					<td class="columna1">Fórmula</td>					
					<td class="columna7">Ver</td>
				</tr><!--final class fila1-->
			</thead>
			<tbody>
				<?php
				if (!empty($indicadoresCuantitativos)) {
...
function eliminarIndicadorCualitativo(id, nombre){
    confirmar=confirm("Eliminar Indicador Cualitativo: " + nombre + " Recuerda Una vez Eliminado No podras Recuperarlo"); 
    if (confirmar){
    	var IndicadorCuealitativo 		 = new Object();
		IndicadorCuealitativo.Id      	 = id;
		IndicadorCuealitativo.Nombre      = nombre;
		var DatosJson = JSON.stringify(IndicadorCuealitativo);
		$.post(baseurl + 'indicador/eliminarIndicadorCualitativo',{ 
			MiIndicadorCuealitativo: DatosJson
		},
		...
		window.location.href = "indicadoresCualitativos";
    } else{
    } 
  }  
</script>