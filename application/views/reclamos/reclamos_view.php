		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Responsable</th>
					<th scope="col" class="clmn1">Tipo</th>
					<th scope="col" class="clmn1">Denunciante</th>
					<th scope="col" class="clmn1">No Atendidos</th>
					<th scope="col" class="clmn1">Resueltos</th>
			...
			<tbody>
				...
				if (!empty($reclamos)) {
					foreach ($reclamos as $key):
						$id   = base64_encode($key->IdReclamos);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->ResponsableDeReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->DenuncianteReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NoAtendidosReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ResueltosReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CostoReclamos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>reclamos/editarReclamo/<?php echo $key->IdReclamos;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarReclamo('<?php echo $key->IdReclamos; ?>','<?php echo $key->ResponsableDeReclamos ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarReclamo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdRecl=id;
				$.post('<?php echo base_url(); ?>reclamos/eliminarReclamo',{IdRecl: IdRecl},function(data) {
				location.reload();
				});
			}
		});
