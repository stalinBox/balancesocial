...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>permisosComponentes/nuevoComponente"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				...
				if (!empty($listaComponentes)) {
					foreach ($listaComponentes as $key){
						$id = base64_encode($key->IdComponents); ?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombComp ?></th>
						<th scope="col" class="clmn1"><?php echo $key->IdComp ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoComp ?></th>
						<th scope="col" class="clmn1"><?php echo $key->DescComp ?></th>
						<th scope="col" class="clmn9" style="display:none;">
							<ul class="edittablamd">
									<li> 
										<a href="<?php echo base_url();?>permisosComponentes/editarComponente/<?php echo $key->IdComponents;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>                
							<th class="clmn10" style="display:none">                
								<ul class="borrartablamd">
									<li> 
										<a href="#" onclick="eliminarUsuarioEmpresa('<?php echo $key->IdComponents; ?>','<?php echo $key->IdComponents ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>
						</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarPermisoPerfil(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idPermPerfil=id;
				$.post('<?php echo base_url(); ?>permisos/eliminarPermisoPerfil',{idPermPerfil: idPermPerfil},function(data) {
				location.reload();
				});
			}
		});
