...
<?php 
/*
echo "Permisos Contrato Proveedores ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Tipo de Contrato</th>	
					<th scope="col" class="clmn1">Objetivo</th>	
					<th scope="col" class="clmn1">Fecha Inicio del Contrato</th>	
					<th scope="col" class="clmn1">Fecha Fin del Contrato</th>						
			...
			<tbody>
				<?php 
				if (!empty($proveedoresContratos)) {
					foreach ($proveedoresContratos as $key):
						$id   = base64_encode($key->IdContrato);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombreProveedor ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoContrato ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ObjetoDeContrato ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaInicioContrato ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaFinContrato ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>proveedor/editarProveedorContrato/<?php echo $key->IdContrato;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarProveedorContrato('<?php echo $key->IdContrato; ?>','<?php echo $key->ObjetoDeContrato ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarProveedorContrato(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdCont=id;
				$.post('<?php echo base_url(); ?>proveedor/eliminarProveedorContrato',{IdCont: IdCont},function(data) {
				location.reload();
				});
			}
		});
