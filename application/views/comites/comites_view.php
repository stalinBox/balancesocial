		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th style="display:none">IdComite</th>
					<th >Nombre</th>
					<th >Tipo</th>
					<th >Integrantes</th>
					<th >Funciones</th>
			...
			<tbody>
				...
				if (!empty($comites)) {
					foreach ($comites as $key):
						$id   = base64_encode($key->IdComite);
					?>
					<tr class="horizontalcuerpo">
						<th  style="display:none"><?php echo $key->IdComite ?></th>
						<th ><?php echo $key->NombreComite ?></th>
						<th ><?php echo $key->TipoComite ?></th>
						<th ><?php echo $key->IntegrantesComite ?></th>
						<th ><?php echo $key->FuncionComite ?></th>
						<th ><?php echo $key->Periodo ?></th>
						<th style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>comite/editarComite/<?php echo $key->IdComite;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarComite('<?php echo $key->IdComite; ?>','<?php echo $key->NombreComite ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Organización</th>
							<th class="th-sm">Estado</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('comite/filtroAnioComite','comite/filtroAnioGuardarComite')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarComite(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdCom = id;
				$.post('<?php echo base_url(); ?>comite/eliminarComite',{IdCom: IdCom},function(data) {
				location.reload();
				});
			}
		});
