...
<?php 
/*
echo "Permisos Beneficio Laboral del Empleado ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1" style="display:none">IdBeneficio</th>
					<th scope="col" class="clmn1">Tipo</th>
					<th scope="col" class="clmn1">Empleados Con Beneficio</th>
					<th scope="col" class="clmn1">Costo que Cubre la Empresa</th>
					<th scope="col" class="clmn1">Días con el Beneficio</th>
			...
			<tbody>
				<?php 
				if (!empty($beneficiosLaborales)) {
					foreach ($beneficiosLaborales as $key):
						$id   = base64_encode($key->IdBeneficio);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1" style="display:none"><?php echo $key->IdBeneficio ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoBeneficioLaboral ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumEmpleadosConServicio ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CostoTotalQueCubreEmpresa ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumDiasOtorgadoElBeneficio ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>empleado/editarBeneficioLaboral/<?php echo $key->IdBeneficio;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarBeneficioLaboral('<?php echo $key->IdBeneficio; ?>','<?php echo $key->TipoBeneficioLaboral ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarBeneficioLaboral(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdBeneficioLab=id;
				$.post('<?php echo base_url(); ?>empleado/eliminarBeneficioLaboral',{IdBeneficioLab: IdBeneficioLab},function(data) {
				location.reload();
				});
			}
		});
