...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>imagenInstitucional/nuevaImagenInstitucional"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Eslogan</th>
			...
			<tbody>
				<?php 
				if (!empty($imagenesInstitucionales)) {
					foreach ($imagenesInstitucionales as $key):
						$id   = base64_encode($key->IdImgInstucional);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->Eslogan ?></th>
						...
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>imagenInstitucional/editarImagenInstitucional/<?php echo $key->IdImgInstucional;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarImagenInstitucional('<?php echo $key->IdImgInstucional; ?>','<?php echo $key->Periodo ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarImagenInstitucional(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idImg=id;
				$.post('<?php echo base_url(); ?>imagenInstitucional/eliminarImagenInstitucional',{idImg: idImg},function(data) {
				location.reload();
				});
			}
		});
