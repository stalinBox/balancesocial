	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				<?php 
				if (!empty($organismos)) {
					foreach ($organismos as $key):
						$id   = base64_encode($key->IdOrganismo);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col"><?php echo $key->NombreOrganismo ?></th>
						<th scope="col""><?php echo $key->DescripcionOrganismo ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>organismo/editarOrganismo/<?php echo $key->IdOrganismo;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarOrganismo('<?php echo $key->IdOrganismo; ?>','<?php echo $key->NombreOrganismo ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
						   		</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarOrganismo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdOrga = id;
				$.post('<?php echo base_url(); ?>empresa/eliminarObjetivo',{IdOrga: IdOrga},function(data) {
				location.reload();
				});
			}
		});
