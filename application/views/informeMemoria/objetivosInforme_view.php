...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>informeMemoria/nuevoObjetivoInformeMemoria"
	}
	function regresar(){
...
<?php 
/*
echo "Permisos Objetivos del Informe";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th scope="col" class="clmn1">Objetivo de la Organización</th>
				...
				<tbody>
					<?php 
					if (!empty($objetivosInforme)) {
						foreach ($objetivosInforme as $key): 
							$id   = base64_encode($key->IdObjetivosInforme); ?>
						<tr class="horizontalcuerpo">
							<th scope="col" class="clmn1"><?php echo $key->Objetivo ?></th>
							<th scope="col" class="clmn1"><?php echo $key->Tipo ?></th>
							<th scope="col" class="clmn1"><?php echo $key->DescripcionCumplimiento ?></th>							
				...
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>informeMemoria/editarObjetivoInformeMemoria/<?php echo $key->IdObjetivosInforme;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarObjetivoInforme('<?php echo $key->IdObjetivosInforme; ?>','<?php echo $key->Objetivo; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
        ...
        <div class="modal-content">
           	...
           	<div class="modal-body">
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					...
					<tbody>
					    <body onload="viewdata('informeMemoria/filtroAnioObjInf','informeMemoria/filtroAnioGuardarObjInf')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarObjetivoInforme(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idObjetivInf=id;
				$.post('<?php echo base_url(); ?>informeMemoria/eliminarObjetivoInforme',{idObjetivInf: idObjetivInf},function(data) {
				location.reload();
				});
			}
		});
