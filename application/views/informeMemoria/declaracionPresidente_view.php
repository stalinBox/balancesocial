...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>informeMemoria/nuevaDeclaracionPresidente"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">						
						<th scope="col" class="clmn1">Declaración</th>
						<th scope="col" class="clmn1">Periodo</th>
						<th scope="col" class="clmn1">Foto</th>
				...
				<tbody>
					<?php 
					if (!empty($declaracionesPresidenteInforme)) {
						foreach ($declaracionesPresidenteInforme as $key): 
							$id   = base64_encode($key->IdDPDescInforme); ?>
						<tr class="horizontalcuerpo">												
							<th ><?php echo $key->Declaracion ?></th>
							<th ><?php echo $key->Periodo ?></th>
							<th scope="col" class="clmn1">
								<img src="data:image/jpg;base64, <?php echo base64_encode($key->Foto);?>" width="30" height="30" alt="" />								
							</th>
							<th scope="col" class="clmn1"><?php echo $key->FechaSistema ?></th>
							<th scope="col" class="clmn9" style="display:none">
			                  <ul class="edittablamd">
			                    <li> 
			                      <a href="<?php echo base_url();?>informeMemoria/editarDeclaracionPresidente/<?php echo $key->IdDPDescInforme;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
			                    </li>
			                  </ul>
			                </th>
			                ...
			                <th scope="col" class="clmn10" style="display:none">
			                  <ul class="borrartablamd">
			                    <li> 
			                      <a href="#" onclick="eliminarDeclaracionPresidente('<?php echo $key->IdDPDescInforme ?>','<?php echo $key->Declaracion; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
			                    </li>
			                  </ul>
			                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Declaración</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('informeMemoria/filtroAnioMemoria','informeMemoria/filtroAnioGuardarMemoria')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarDeclaracionPresidente(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idDeclaracion=id;
				$.post('<?php echo base_url(); ?>informeMemoria/eliminarDeclaracionPresidente',{idDeclaracion: idDeclaracion},function(data) {
				location.reload();
				});
			}
		});
