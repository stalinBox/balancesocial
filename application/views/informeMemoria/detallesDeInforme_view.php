...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>informeMemoria/nuevoDetalleInformeMemoria"
	}
	function regresar(){
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				<thead>
					<tr class="horizontalazul">
						<th >Número de Declaraciones Emitidas</th>		
						<th >Nombre de la persona que Emite la Declaración</th>
						<th >Cargo de la Persona que Emite la Declaración</th>
				...
				<tbody>
					<?php 
					if (!empty($detallesDeInforme)) {
						foreach ($detallesDeInforme as $key): 
							$id   = base64_encode($key->IdDetallesInforme); ?>
						<tr class="horizontalcuerpo">
							<th ><?php echo $key->NumDeclaracionesEmitidas ?></th>
							<th ><?php echo $key->IdentificacionDeQuienEmite ?></th>
							<th ><?php echo $key->CargoDeQuienEmite ?></th>
							<th ><?php echo $key->FechaEmisionInforme ?></th>
							<th ><?php echo $key->Periodo ?></th>
							<th ><?php echo $key->FechaSistema ?></th>
				<th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>informeMemoria/editarDetalleInformeMemoria/<?php echo $key->IdDetallesInforme;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarDetalleInformeMemoria('<?php echo $key->IdDetallesInforme; ?>','<?php echo $key->IdentificacionDeQuienEmite; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
						</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Número de declaración</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('informeMemoria/filtroAnioDetaInforme','informeMemoria/filtroAnioGuardarDetaInforme')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarDetalleInformeMemoria(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idDetaLogro=id;
				$.post('<?php echo base_url(); ?>informeMemoria/eliminarDetalleInformeMemoria',{idDetaLogro: idDetaLogro},function(data) {
				location.reload();
				});
			}
		});
