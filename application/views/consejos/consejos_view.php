		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1" style="display:none">IdConsejos</th>
					<th scope="col" class="clmn1">Nombre</th>
					<th scope="col" class="clmn1">Tipo</th>
					<th scope="col" class="clmn1">Integrantes</th>					
					<th scope="col" class="clmn1">N. Hombres</th>					
					<th scope="col" class="clmn1">N. Mujeres</th>					
					<th scope="col" class="clmn1">Funciones</th>
			...
			<tbody>
				<?php 
				if (!empty($consejos)) {
					foreach ($consejos as $key):
						$id   = base64_encode($key->IdConsejos);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1" style="display:none"><?php echo $key->IdConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NombreConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->IntegrantesConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumeroHombres ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumeroMujeres ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FuncionConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Periodo ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>consejo/editarConsejo/<?php echo $key->IdConsejos;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarConsejo('<?php echo $key->IdConsejos; ?>','<?php echo $key->NombreConsejos ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Función</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('consejo/filtroAnioConsejo','consejo/filtroAnioGuardarConsejo')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarConsejo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdConsejo=id;
				$.post('<?php echo base_url(); ?>consejo/eliminarConsejos',{IdConsejo: IdConsejo},function(data) {
				location.reload();
				});
			}
		});
