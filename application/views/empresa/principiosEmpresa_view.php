...
*/
 ?>
	...
	<div id="contenedor" style="margin:15px auto">
		...
		<tbody>
		<?php
		if (!empty($principiosEmpresa)) {
			foreach ($principiosEmpresa as $key):  ?>
		<th ><?php echo $key->PrincipioPrincipiosEmpresa ?></th>
		<th ><?php echo $key->DescripcionPrincipiosEmpresa ?></th>
                <th scope="col" class="clmn9" style="display:none">
                  <ul class="edittablamd">
                    <li> 
                      <a href="<?php echo base_url();?>empresa/editarPrincipio/<?php echo $key->IdPrincipiosEmpresa;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
                <th scope="col" class="clmn10" style="display:none">
                  <ul class="borrartablamd">
                    <li> 
                      <a href="#" onclick="eliminarPrincipio('<?php echo $key->IdPrincipiosEmpresa; ?>','<?php echo $key->PrincipioPrincipiosEmpresa ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
                    </li>
                  </ul>
                </th>
	</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarPrincipio(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idPrincipio=id;
				$.post('<?php echo base_url(); ?>empresa/eliminarPrincipio',{idPrincipio: idPrincipio},function(data) {
			location.reload();
		});
			}
		});
