 ...
 ?>
	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
				...
				<tbody>
					<?php 
					if (!empty($valoresEmpresa)) {
						foreach ($valoresEmpresa as $key): ?>
						<tr class="horizontalcuerpo">
							<th  style="display:none"><?php echo $key->IdValoresEmpresa ?></th>
							<th ><?php echo $key->ValorValoresEmpresa ?></th>
							<th ><?php echo $key->DescripcionValoresEmpresa ?></th>
							<th scope="col" class="clmn9" style="display:none">
								<ul class="edittablamd">
									<li> 
										<a href="<?php echo base_url();?>empresa/editarValores/<?php echo $key->IdValoresEmpresa;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>
							<th scope="col" class="clmn10" style="display:none">
								<ul class="borrartablamd">
									<li> 
										<a href="#" onclick="eliminarValor('<?php echo $key->IdValoresEmpresa; ?>','<?php echo $key->ValorValoresEmpresa; ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>
						</tr>
...
<script type="text/javascript">
	...
	function eliminarValor(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm){
				var idValoreEmp = id;
				$.post('<?php echo base_url(); ?>empresa/eliminarValor',{idValoreEmp: idValoreEmp},function(data) {
				location.reload();
				});
			}
		});
