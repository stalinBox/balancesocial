...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>permisos/nuevoPermisoEspecialUsuario"
	}
	function regresar(){
...
<?php 
/*
echo "Permisos Especiales de Usuarios sobre Tablas";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn3">Correo</th>
					<th scope="col" class="clmn4">Dirección</th>
					<th scope="col" class="clmn5">Teléfono</th>
					<th scope="col" class="clmn6">Celular</th>
					<th scope="col" class="clmn7">Perfil</th>
					<th scope="col" class="clmn7">Permisos Especiales</th>
			...
			<tbody>
				<?php 
				if (!empty($usuariosEmpresa)) {
					foreach ($usuariosEmpresa as $key) {
						$id   = base64_encode($key->IdUsuario); ?>
						<tr class="horizontalcuerpo">
							...
							<th scope="col" class="clmn3"><?php echo $key->MailUsuario; ?></th>
							<th scope="col" class="clmn4"><?php echo $key->DireccionUsuario; ?></th>
							<th scope="col" class="clmn5"><?php echo $key->TelefonoUsuario; ?></th>
							<th scope="col" class="clmn6"><?php echo $key->CelularUsuario; ?></th>
							<th scope="col" class="clmn7"><?php echo $key->Perfil; ?></th>
							<th scope="col" class="clmn7"><?php echo $key->Permisos; ?></th>
							<th scope="col" class="clmn9" style="display:none">
								<ul class="edittablamd">
									<li> 
										<a href="<?php echo base_url();?>permisos/verPermisosEspecialesAUsuario/<?php echo $key->IdUsuario;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>                
							<th class="clmn10" style="display:none">                
								<ul class="borrartablamd">
									<li> 
										<a href="#" onclick="eliminarUsuarioEmpresa('<?php echo $key->IdUsuario; ?>','<?php echo $key->NombreUsuario ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
									</li>
								</ul>
							</th>
						</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarUsuarioEmpresa(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idSocio=id;
				$.post('<?php echo base_url(); ?>empresa/eliminarUsuarioEmpresa',{idUser: idUser},function(data) {
				location.reload();
				});
			}
		});
