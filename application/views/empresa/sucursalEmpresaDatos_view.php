...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>empresa/nuevoDatosSucursalEmpresa"
	}
	function regresar(){
		window.location="<?php echo base_url()?>empresa/modulosSucursales";
	}
	function denegado() {
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">N. de Empleados</th>				
					<th scope="col" class="clmn1">Captacion Del Público</th>				
					<th scope="col" class="clmn1">Porcentaje de Participación</th>				
					<th scope="col" class="clmn1">Miembros de Consejos</th>				
					<th scope="col" class="clmn1">Miembros Cons. Ofic. Signif.</th>	
					<th scope="col" class="clmn1">Cumplimiento de Presumuesto</th>	
			...
			<tbody>
				...
				if (!empty($sucursalesEmpresa)) {
					foreach ($sucursalesEmpresa as $key):
						$id   = base64_encode($key->IdSucursal);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->Nombre ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CantidadEmpleadosSucursal ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CaptacionDelPublico ?></th>
						<th scope="col" class="clmn1"><?php echo $key->PorcentajeParticipacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TotalMiembrosConsejos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TotalVocalesRepresentantes ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CumplePresupuesto ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>empresa/editarDatosSucursalEmpresa/<?php echo $key->IdSucursal;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarDatosSucursalEmpresa('<?php echo $key->IdSED; ?>','<?php echo $key->Nombre ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarDatosSucursalEmpresa(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSuc = id;
				$.post('<?php echo base_url(); ?>empresa/eliminarDatosSucursalEmpresa',{IdSuc: IdSuc},function(data) {
				location.reload();
				});
			}
		});
