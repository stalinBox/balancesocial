...
<?php 
/*
echo "Permisos Programas de Concientización ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Costo Planificado</th>
					<th scope="col" class="clmn1">Estado</th>
					<th scope="col" class="clmn1">Convenio</th>
					<th scope="col" class="clmn1">Fecha Planificación</th>
					<th scope="col" class="clmn1">Fecha de Inicio</th>
			...
			<tbody>
				<?php 
				if (!empty($programasConcientizacion)) {
					foreach ($programasConcientizacion as $key):
						$id   = base64_encode($key->IdProgramasConcientizacion);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombrePrograma ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoPrograma ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CostoPlanificado ?></th>
						<th scope="col" class="clmn1"><?php echo $key->EstadoPrograma ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ConvenioPrograma ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaPlanificacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaInicioPrograma ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Periodo ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>programaConcientizacion/editarProgConcien/<?php echo $key->IdProgramasConcientizacion;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarProgConcien('<?php echo $key->IdProgramasConcientizacion; ?>','<?php echo $key->NombrePrograma ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
		        ...
		        <table id="mytableModal" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
					<thead >
					    <tr>    
							...
							<th class="th-sm">Costo Planificado</th>
							<th class="th-sm">Periodo</th>
					    </tr>
					</thead>
					<tbody>
					    <body onload="viewdata('programaConcientizacion/filtroAnioProgConcientizacion','programaConcientizacion/filtroAnioGuardarProgConcientizacion')">
					    </body>
					</tbody>
		        </table>
           	</div>
...
<script type="text/javascript">
	...
	function eliminarProgConcien(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdProgramasCon = id;
				$.post('<?php echo base_url(); ?>programaConcientizacion/eliminarProgConcien',{IdProgramasCon: IdProgramasCon},function(data) {
				location.reload();
				});
			}
		});
