...
<?php 
/*
echo "Permisos Sesiones de Consejos ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Código de Consejo</th>
					<th scope="col" class="clmn1">Tipo</th>
					<th scope="col" class="clmn1">Fecha Planificada</th>
					<th scope="col" class="clmn1">Asistentes Planificados</th>
			...
			<tbody>
				...
				if (!empty($sesionesConsejos)) {
					foreach ($sesionesConsejos as $key):
						$id   = base64_encode($key->IdReuniones);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombreReunion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->IdConsejo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoSesion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaPlanificacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->RepresentantesPlanificados ?></th>
						<th scope="col" class="clmn1"><?php echo $key->EstadoReunion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->RepresentantesPresentes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>sesionConsejo/editarSesionConsejo/<?php echo $key->IdReuniones;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSesionConsejo('<?php echo $key->IdReuniones; ?>','<?php echo $key->NombreReunion ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarSesionConsejo(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdReun = id;
				$.post('<?php echo base_url(); ?>sesionConsejo/eliminarSesionConsejo',{IdReun: IdReun},function(data) {
				location.reload();
				});
			}
		});
