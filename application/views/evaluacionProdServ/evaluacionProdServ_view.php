...
<?php 
/*
echo "Permisos Evaluaciones de Productos y Servicios ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">N.</th>
					<th scope="col" class="clmn1">Frecuencia</th>
					<th scope="col" class="clmn1">Objetivo</th>
					<th scope="col" class="clmn1">Evaluaciones Efectuadas</th>
			...
			<tbody>
				<?php 
				if (!empty($evaluacionProdServ)) {
					foreach ($evaluacionProdServ as $key):
						$id   = base64_encode($key->IdEPS);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NumEvaProdServProgramado ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FrecuenciaEvaluacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Objetivo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumEvaProdSerEfectuados ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>evaluacionProdServ/editarEvaluacionProdServ/<?php echo $key->IdEPS;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarEvaluacion('<?php echo $key->IdEPS; ?>','<?php echo $key->NumEvaProdServProgramado ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarEvaluacion(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdEP=id;
				$.post('<?php echo base_url(); ?>evaluacionProdServ/eliminarEvaluacionProdServ',{IdEP: IdEP},function(data) {
				location.reload();
				});
			}
		});
