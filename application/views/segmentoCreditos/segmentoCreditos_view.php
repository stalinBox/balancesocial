...
<script type="text/javascript">
	...
	function regresar(){
		window.location="<?php echo base_url()?>segmentoCredito";
	}
	function denegado() {
...
<?php 
/*
echo "Permisos Segmento de Crédito ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Ejecución (%)</th>
					<th scope="col" class="clmn1">Establecido por POA (%)</th>
					<th scope="col" class="clmn1">Cumplimiento</th>
			...
			<tbody>
				<?php 
				if (!empty($segmentoCreditos)) {
					foreach ($segmentoCreditos as $key):
						$id   = base64_encode($key->IdSegmentosCredito);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombreSegmento ?></th>
						<th scope="col" class="clmn1"><?php echo $key->MontoConcedido ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Ejecucion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->EstablecidoPorPOA ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Cumplimiento ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>segmentoCredito/editarSegmentoCredito/<?php echo $key->IdSegmentosCredito;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSegmentoCredito('<?php echo $key->IdSegmentosCredito; ?>','<?php echo $key->NombreSegmento ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarSegmentoCredito(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSegmentosCredit=id;
				$.post('<?php echo base_url(); ?>segmentoCredito/eliminarSegmentoCredito',{IdSegmentosCredit: IdSegmentosCredit},function(data) {
				location.reload();
				});
			}
		});
