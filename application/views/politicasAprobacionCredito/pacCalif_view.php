...
<script type="text/javascript">
	...
	function regresar(){
		window.location="<?php echo base_url()?>politicasAprobacionCredito";
	}
	function denegado() {
...
<?php 
/*
echo "Permisos Calificación de Políticas ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Selección</th>
			...
			<tbody>
				<?php 
				if (!empty($pacCalif)) {
					foreach ($pacCalif as $key):
						$id   = base64_encode($key->IdPACCalif);
					?>
					<tr class="horizontalcuerpo">
						...
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>politicasAprobacionCredito/editarPACCalif/<?php echo $key->IdPACCalif;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarPACCalif('<?php echo $key->IdPACCalif; ?>','<?php echo $key->Pregunta ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarPACCalif(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdPACCal=id;
				$.post('<?php echo base_url(); ?>politicasAprobacionCredito/eliminarPACCalif',{IdPACCal: IdPACCal},function(data) {
				location.reload();
				});
			}
		});
