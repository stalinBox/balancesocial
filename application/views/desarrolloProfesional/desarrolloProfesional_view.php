...
<script type="text/javascript">
	...
	function regresar(){
		window.location="<?php echo base_url()?>desarrolloProfesional";
	}
</script>
...
<div id="MANIPULACIONDATOS">
	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				<?php 
				if (!empty($desarrolloProfesional)) {
					foreach ($desarrolloProfesional as $key):
						$id   = base64_encode($key->IdDesarrolloProfesional);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NombreDesarrolloProfesional ?></th>
						<th scope="col" class="clmn2"><?php echo $key->Descripcion ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>desarrolloProfesional/editarDesarrolloProfesional/<?php echo $key->IdDesarrolloProfesional;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarDesarrolloProfesional('<?php echo $key->IdDesarrolloProfesional; ?>','<?php echo $key->NombreDesarrolloProfesional ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarDesarrolloProfesional(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdDesarrolloProf = id;
				$.post('<?php echo base_url(); ?>desarrolloProfesional/eliminarDesarrolloProfesional',{IdDesarrolloProf: IdDesarrolloProf},function(data) {
				location.reload();
				});
			}
		});
