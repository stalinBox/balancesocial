      ...
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
        <thead>        
          <tr class="horizontalcabpdf">
           <th >Número Total de Empleados Capacitados</th>
           <th >Número de horas de Capacitación</th>
           <th >Monto Total Invertido en Capacitaciones</th>
           <th >Número de Tipos de Capacitación</th>
           <th >Periodo</th>
         </tr>
       </thead>
       <tbody > ';       
       if (!empty($capacitacionResumen))
         foreach ($capacitacionResumen as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->Capacitados.'</td>
            <td scope="col" >'.$key->Horas.'</td>            
            <td scope="col" >'.number_format($key->MontoInvertido, 2).'</td>            
            <td scope="col" >'.$key->TiposCapacitacion.'</td>
