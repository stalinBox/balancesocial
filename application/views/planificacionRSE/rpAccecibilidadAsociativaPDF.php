       ...
       if (!empty($socios))
         foreach ($socios as $key){
          ...
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.number_format($key->NumTSociosActivos, 0).'</td>
            <td scope="col" >'.number_format($key->NumTSociosActivosMujeres, 0).'</td>
            <td scope="col" >'.number_format($key->SociosHombre, 0).'</td>
            <td scope="col" >'.number_format($key->NumTPersonasJuridicasIncorporadas, 0).'</td>
            <td scope="col" >'.number_format($key->ValorTPorCertificadosDeAportacion, 0).'</td>
            <td scope="col" >'.$key->Periodo.'</td>        
            <td scope="col" >'.$key->FechaSistema.'</td>        
          </tr>
          ';
        }
        $html .='
