  ...
  <div id="tablapdf">
    <div id="tablaceldaspdf">
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
       ...
       <tbody > ';       
       if (!empty($promedioSegmentosCreditos))
         foreach ($promedioSegmentosCreditos as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->NombreSegmento.'</td>
            <td scope="col" >'.$key->TipoSegmentoCredito.'</td>
            <td scope="col" >'.number_format($key->MontoColocado, 2).'</td>
            <td scope="col" >'.number_format($key->Operaciones, 0).'</td>
