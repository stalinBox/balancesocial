  ...
  <div id="tablapdf">
    <div id="tablaceldaspdf">
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
       ...
       <tbody > ';       
       if (!empty($acuerdos))
         foreach ($acuerdos as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->OrganizacionConvenio.'</td>
            <td scope="col" >'.$key->TipoOrganizacion.'</td>
            <td scope="col" >'.$key->NombreAcuerdo.'</td>
            <td scope="col" >'.$key->FechaPlanificacion.'</td>
            <td scope="col" >'.number_format($key->CostoPlanificado, 2).'</td>
            <td scope="col" >'.$key->Estado.'</td>
            <td scope="col" >'.$key->FechaEjecucion.'</td>
            <td scope="col" >'.$key->FechaFinalizacion.'</td>
            <td scope="col" >'.number_format($key->CostoEjecutado, 2).'</td>
            <td scope="col" >'.$key->Beneficiarios.'</td>
