  ...
  <div id="tablapdf">
    <div id="tablaceldaspdf">
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
       ...
       <tbody > ';       
       if (!empty($contratosProveedores))
         foreach ($contratosProveedores as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->TipoContrato.'</td>
            <td scope="col" >'.$key->NombreProveedor.'</td>
            <td scope="col" >'.$key->ObjetoDeContrato.'</td>
            <td scope="col" >'.$key->FechaInicioContrato.'</td>
            <td scope="col" >'.$key->FechaFinContrato.'</td>
            <td scope="col" >'.$key->ObligacionDePartes.'</td>
          </tr>
          ';
        }
        $html .='
