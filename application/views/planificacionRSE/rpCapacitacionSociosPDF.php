  ...
  <div id="tablapdf">
    <div id="tablaceldaspdf">
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
       ...
       <tbody > ';       
       if (!empty($capacitacionesSocios))
         foreach ($capacitacionesSocios as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->NombreCapacitacionSocios.'</td>
            <td scope="col" >'.$key->FechaPlanificacion.'</td>
            <td scope="col" >'.$key->TipoCapacitacionSocios.'</td>
            <td scope="col" >'.number_format($key->CostoPresupuestado, 2).'</td>
            <td scope="col" >'.$key->HorasPlanificadas.'</td>
            <td scope="col" >'.$key->ParticipantesEsperado.'</td>
            <td scope="col" >'.$key->EstadoCapacitacion.'</td>
            <td scope="col" >'.$key->FechaEjecucion.'</td>
            <td scope="col" >'.$key->NumeroAsistentes.'</td>
            <td scope="col" >'.$key->HorasEjecutadas.'</td>
            <td scope="col" >'.number_format($key->CostoCapacitacion, 2).'</td>
