      ...
      <table border="1" cellspacing="0px" width="100%" class="tablclpelpdf">
        <thead>        
          <tr class="horizontalcabpdf">
           <th >Nombre de Segmento</th>
           <th >Monto Concedido</th>
           <th >Ejecución</th>
           <th >Establecido por el POA</th>
           <th >Cumplimiento</th>
           <th >Fecha de Registro</th>
         </tr>
       </thead>
       <tbody > ';       
       if (!empty($segmentosDeCredito))
         foreach ($segmentosDeCredito as $key){
          $html .= '
          <tr class="horizontalpdfcuerpo1">
            <td scope="col" >'.$key->NombreSegmento.'</td>
            <td scope="col" >'.number_format($key->MontoConcedido, 2).'</td>
            <td scope="col" >'.number_format($key->Ejecucion, 2).'</td>
            <td scope="col" >'.number_format($key->EstablecidoPorPOA, 2).'</td>
            <td scope="col" >'.$key->Cumplimiento.'</td>
