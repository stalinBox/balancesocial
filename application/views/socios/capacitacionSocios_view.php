...
<?php 
/*
echo "Permisos Capacitación a Socios ";
echo @$permisos[0]->Ver;
...
<div id="MANIPULACIONDATOS">
	<div id="menumanipulaciondatos">
		...
		<ul class="manipdatos2">
			<li> 
				<a href="#" <?php if (@$permisos[0]->Actualizar == 1 OR @$admin == 1) { echo 'onclick="ocultarColumna(10,true)"';}else{echo 'onclick="denegado()"';} ?> title="Editar"><img src="<?php echo base_url()?>imagenes/EDITAR.png" width="20" height="20" alt=""/></a>
			</li>
		</ul>
		<ul class="manipdatos">
			<li> 
				<a href="#" <?php if (@$permisos[0]->Eliminar == 1 OR @$admin == 1) { echo 'onclick="ocultarColumna(11,true)"';}else{echo 'onclick="denegado()"';} ?> title="Descartar"><img src="<?php echo base_url()?>imagenes/BORRAR-MENU.png" width="20" height="20" alt=""/></a>
			</li>
		</ul>
		<ul class="manipdatos2">
			<li> 
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1" style="display:none">IdCapacitacionSocios</th>
					...
					<th scope="col" class="clmn1">Participantes Esperado</th>
			...
			<tbody>
				<?php 
				if (!empty($capacitacionSocios)) {
					foreach ($capacitacionSocios as $key):
						$id   = base64_encode($key->IdCapacitacionSocios);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1" style="display:none"><?php echo $key->IdCapacitacionSocios ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NombreCapacitacionSocios ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaPlanificacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoCapacitacionSocios ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CostoPresupuestado ?></th>
						<th scope="col" class="clmn1"><?php echo $key->HorasPlanificadas ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ParticipantesEsperado ?></th>
						<th scope="col" class="clmn1"><?php echo $key->EstadoCapacitacion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumeroAsistentes ?></th>
						<th scope="col" class="clmn1"><?php echo $key->HorasEjecutadas ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>socio/editarCapacitacionSocio/<?php echo $key->IdCapacitacionSocios;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarCapacitacionSocio('<?php echo $key->IdCapacitacionSocios; ?>','<?php echo $key->NombreCapacitacionSocios ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarCapacitacionSocio(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdCapSo=id;
				$.post('<?php echo base_url(); ?>socio/eliminarCapacitacionSocio',{IdCapSo: IdCapSo},function(data) {
				location.reload();
				});
			}
		});
