		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">					
					<th scope="col" class="clmn1">Incorporados</th>
					<th scope="col" class="clmn2">Personas Naturales</th>					
					<th scope="col" class="clmn2">Personas Jurídicas</th>					
					<th scope="col" class="clmn2">Retirados</th>
					<th scope="col" class="clmn3">Activos</th>					
					<th scope="col" class="clmn3">Mes</th>					
			...
			<tbody>
				...
				if (!empty($socios)) {
					foreach ($socios as $key):
						$id   = base64_encode($key->IdSocios);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->NumSociosIncorporados ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumPersonasNaturalesIncorporadas ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumPersonasJuridicasIncorporadas ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumSociosRetirados ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NumSociosActivos ?></th>
						<th scope="col" class="clmn1"><?php echo $key->MesPertenece ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>socio/editarSocio/<?php echo $key->IdSocios;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSocio('<?php echo $key->IdSocios; ?>','<?php echo $key->IdSocios ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarSocio(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idSocio=id;
				$.post('<?php echo base_url(); ?>socio/eliminarSocio',{idSocio: idSocio},function(data) {
				location.reload();
				});
			}
		});
