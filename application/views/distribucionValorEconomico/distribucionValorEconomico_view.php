...
<?php 
/*
echo "Permisos Distribución Valor Económico ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn1">Sub Tipo</th>
					<th scope="col" class="clmn1">Nombre de Cuenta</th>
					<th scope="col" class="clmn1">Saldo</th>
			...
			<tbody>
				<?php 
				if (!empty($distribucionValorEconomico)) {
					foreach ($distribucionValorEconomico as $key):
						$id   = base64_encode($key->IdDVE);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->TipoDistribucion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->SubTipoDistribucion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->NombreSubCuenta ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Saldo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>distribucionValorEconomico/editarDistribucionesValorEconomico/<?php echo $key->IdDVE;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarDistribucionValorEconomico('<?php echo $key->IdDVE; ?>','<?php echo $key->TipoDistribucion ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarDistribucionValorEconomico(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdDV=id;
				$.post('<?php echo base_url(); ?>distribucionValorEconomico/eliminarDistribucionValorEconomico',{IdDV: IdDV},function(data) {
				location.reload();
				});
			}
		});
