...
<?php 
/*
echo "Permisos Satisfacción Clientes ";
echo @$permisos[0]->Ver;
...
<div id="MANIPULACIONDATOS">
	...
	<div id="contenedor" style="margin:15px auto">
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			...
			<tbody>
				<?php 
				if (!empty($satisfaccionClientes)) {
					foreach ($satisfaccionClientes as $key):
						$id   = base64_encode($key->IdSatisfaccionCliente);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->TipoSatisfaccion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Metodo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ResultadoSatisfaccion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ObservacionSatisfaccion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Frecuencia ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>satisfaccionCliente/editarSatisfaccionCliente/<?php echo $key->IdSatisfaccionCliente;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSatisfaccionCliente('<?php echo $key->IdSatisfaccionCliente; ?>','<?php echo $key->TipoSatisfaccion ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarSatisfaccionCliente(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSatisfaccionClient=id;
				$.post('<?php echo base_url(); ?>satisfaccionCliente/eliminarSatisfaccionCliente',{IdSatisfaccionClient: IdSatisfaccionClient},function(data) {
				location.reload();
				});
			}
		});
