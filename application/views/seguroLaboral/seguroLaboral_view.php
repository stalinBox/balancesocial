		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<!-- <th scope="col" class="clmn1">Proveedor</th> -->
					<th scope="col" class="clmn1">Monto</th>
					<th scope="col" class="clmn1">Fecha de Adquisición</th>
					<th scope="col" class="clmn1">Años de Vigencia</th>
					<th scope="col" class="clmn1">Siniestro Ocurrido</th>
					<th scope="col" class="clmn1">Valor Cubierto</th>
			...
			<tbody>
				<?php 
				if (!empty($segurosLaboral)) {
					foreach ($segurosLaboral as $key):
						$id   = base64_encode($key->IdSeguroLaboral);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->TipoSeguroL ?></th>
						<!-- <th scope="col" class="clmn1"><?php echo $key->ProveedorSeguroL ?></th> -->
						<th scope="col" class="clmn1"><?php echo $key->MontoCubreSeguroL ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaAdquiereSeguroL ?></th>
						<th scope="col" class="clmn1"><?php echo $key->VigenciaAniosSeguroL ?></th>
						<th scope="col" class="clmn1"><?php echo $key->SiniestroOcurrido ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ValorRealSiniestro ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>seguroLaboral/editarSeguroLaboral/<?php echo $key->IdSeguroLaboral;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSeguroLaboral('<?php echo $key->IdSeguroLaboral; ?>','<?php echo $key->TipoSeguroL ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
<script type="text/javascript">
	...
	function eliminarSeguroLaboral(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSeguroLab = id;
				$.post('<?php echo base_url(); ?>seguroLaboral/eliminarSeguroLaboral',{IdSeguroLab: IdSeguroLab},function(data) {
				location.reload();
				});
			}
		});
