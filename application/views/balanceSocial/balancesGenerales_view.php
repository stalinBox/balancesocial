...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>balanceSocial/subirBalance";
	}
	function regresar(){
...
<?php 
/*
echo "Permisos Importar Balance General ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Balance General</th>					
					<th scope="col" class="clmn1">Nombre del Archivo</th>
			...
			<tbody>
				<?php 
				if (!empty($balancesGenerales)) {
					foreach ($balancesGenerales as $key):
						$id   = base64_encode($key->IdBalanceMaestro);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->Nombre ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Archivo ?></th>
						...
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>balanceSocial/editarAcuerdo/<?php echo $key->IdBalanceMaestro;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarBalanceGeneral('<?php echo $id; ?>','<?php echo $key->Nombre ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarBalanceGeneral(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var idBalGen=id;
				$.post('<?php echo base_url(); ?>balanceSocial/eliminarBalanceGeneral',{idBalGen: idBalGen},function(data) {
				location.reload();
				});
			}
		});
