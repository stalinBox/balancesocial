		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1">Sanción</th>
			...
			<tbody>
				<?php 
				if (!empty($sanciones)) {
					foreach ($sanciones as $key):
						$id   = base64_encode($key->IdSanciones);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->TipoSancion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->TipoMulta ?></th>
						<th scope="col" class="clmn1"><?php echo $key->ParteDenunciante ?></th>
						<th scope="col" class="clmn1"><?php echo $key->CostoSancion ?></th>
						<th scope="col" class="clmn1"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>sancion/editarSancion/<?php echo $key->IdSanciones;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarSancion('<?php echo $key->IdSanciones; ?>','<?php echo $key->TipoSancion ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarSancion(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdSancion=id;
				$.post('<?php echo base_url(); ?>sancion/eliminarSancion',{IdSancion: IdSancion},function(data) {
				location.reload();
				});
			}
		});
