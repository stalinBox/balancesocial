...
<script type="text/javascript">
	...
	function nuevo(){
		window.location="<?php echo base_url()?>importAudiInterBalanceSocial/nuevoIAISBSCalif"
	}
	function regresar(){
		window.location="<?php echo base_url()?>importAudiInterBalanceSocial";
	}
	function denegado() {
...
<?php 
/*
echo "Permisos Calificación sobre la Imprtancia de Auditoría interna de balance Social ";
echo @$permisos[0]->Ver;
		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					...
					<th scope="col" class="clmn2">Calificación</th>
			...
			<tbody>
				<?php 
				if (!empty($iaisbsCalif)) {
					foreach ($iaisbsCalif as $key):
						$id   = base64_encode($key->IdIAISBSCalif);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1"><?php echo $key->Pregunta ?></th>
						<th scope="col" class="clmn2"><?php echo $key->Calificacion ?></th>
						<th scope="col" class="clmn3"><?php echo $key->FechaMes ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>importAudiInterBalanceSocial/editarIAISBSCalif/<?php echo $key->IdIAISBSCalif;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarIAISBSCalif('<?php echo $key->IdIAISBSCalif; ?>','<?php echo $key->Pregunta ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarIAISBSCalif(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdIAISBSCal=id;
				$.post('<?php echo base_url(); ?>importAudiInterBalanceSocial/eliminarIAISBSCalif',{IdIAISBSCal: IdIAISBSCal},function(data) {
				location.reload();
				});
			}
		});
