		...
		<table id="tablaPaginada" class="table table-bordered table-striped table-hover text-centered table-sm" style="width:100%">
			<thead>
				<tr class="horizontalazul">
					<th scope="col" class="clmn1" style="display:none">IdAhorro</th>
					<th scope="col" class="clmn1">Iniciativa</th>
					<th scope="col" class="clmn1">Individual</th>
					<th scope="col" class="clmn1">Colectivo</th>
					<th scope="col" class="clmn1">Diferencia</th>
					<th scope="col" class="clmn1">Porcentage de Ahorro</th>					
			...
			<tbody>
				...
				if (!empty($ahorroEnEscala)) {
					foreach ($ahorroEnEscala as $key):
						$id   = base64_encode($key->IdAhorro);
					?>
					<tr class="horizontalcuerpo">
						<th scope="col" class="clmn1" style="display:none"><?php echo $key->IdAhorro ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Iniciativa ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Individual ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Colectivo ?></th>
						<th scope="col" class="clmn1"><?php echo $key->Diferencia ?></th>
						<th scope="col" class="clmn1"><?php echo $key->PorcentajeAhorro ?></th>
						<th scope="col" class="clmn9" style="display:none">
							<ul class="edittablamd">
								<li> 
									<a href="<?php echo base_url();?>ahorroEnEscala/editarAhorroEnEscala/<?php echo $key->IdAhorro;?>"><img src="<?php echo base_url();?>imagenes/ACTUALIZAR.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
						<th scope="col" class="clmn10" style="display:none">
							<ul class="borrartablamd">
								<li> 
									<a href="#" onclick="eliminarAhorroEnEscala('<?php echo $key->IdAhorro; ?>','<?php echo $key->Iniciativa ?>');"><img src="<?php echo base_url();?>imagenes/BORRAR1.png" width="30" height="28" alt=""/></a>
								</li>
							</ul>
						</th>
					</tr>
...
// LLAMADA A LA FUNCION ELIMINAR Y LA VENTANA MODAL
	function eliminarAhorroEnEscala(id, nombre){
		...
		modalConfirm(function(confirm){
			if (confirm==true){
				var IdAhorr=id;
				$.post('<?php echo base_url(); ?>ahorroEnEscala/eliminarAhorroEnEscala',{IdAhorr: IdAhorr},function(data) {
				location.reload();
				});
			}
		});
